...
export default function Dashboard() {
  ...
  const inTransitOrders = orders.filter(
    (order: Order) => order.status === "in transit"
  ).length;
  ...
  const statCards = [
    {
      ...
      sub: `${pendingOrders} pending, ${inTransitOrders} in transit`,
    },
    {
      icon: Clock,
    ...
    <Layout>
      ...
      >
        <div className="container mx-auto px-4 flex justify-between items-center">
          ...
          >
            <Link href="/catalog">
              <Button>{t('dashboard.browse_products')}</Button>
            </Link>
          </motion.div>
        </div>
      </motion.div>
              ...
              <Tabs defaultValue="products" className="w-full">
                <TabsList className="grid w-full grid-cols-3 mb-6 overflow-x-auto">
                  <TabsTrigger value="products" className="text-xs sm:text-sm">
                    ...
                    <span className="hidden sm:inline">{t('dashboard.browse_products_tab')}</span>
                    <span className="sm:hidden">Products</span>
                    <span className="ml-1">({cart.length})</span>
                  </TabsTrigger>
                  <TabsTrigger value="orders" className="text-xs sm:text-sm">
                    <span className="hidden sm:inline">{t('dashboard.order_history')}</span>
                    <span className="sm:hidden">Orders</span>
                  </TabsTrigger>
                  <TabsTrigger value="contact" className="text-xs sm:text-sm">
                    <MessageCircle className="mr-1 sm:mr-2 h-3 w-3 sm:h-4 sm:w-4" />
                    <span className="hidden sm:inline">{t('dashboard.contact_support')}</span>
                  ...
                  {cart.length > 0 && (
                    <div className="bg-accent/10 border border-accent/20 rounded-lg p-6 space-y-4">
                      <h4 className="font-bold text-primary text-lg">
                        {t('dashboard.current_cart')}
                      </h4>
                      <div className="space-y-3 max-h-[400px] overflow-y-auto">
                        {cart.map((item) => (
                          <div
                            key={item.product.id}
                            className="flex flex-col sm:flex-row sm:justify-between sm:items-center p-3 bg-white rounded-md border border-border gap-3"
                          >
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow>
                          ...
                          <TableHead className="min-w-[100px]">{t('table.date')}</TableHead>
                          <TableHead className="min-w-[200px]">{t('table.items')}</TableHead>
                        ...
                        <CardContent className="space-y-4">
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground mb-1">
                              For product inquiries, quotations, and bulk
                              orders.
                            </p>
                          ...
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground mb-1">
                              For order tracking, modifications, and shipping
                              updates.
                            </p>
                            <p className="text-sm font-medium break-all">
                              orders@kinsa.com
